...
import { GAME_WIDTH, GAME_HEIGHT, FALLING_ITEMS, PLAYER_WIDTH, PLAYER_HEIGHT, ITEM_SIZE, POWER_UPS } from '../constants.js';
import { PowerUpType } from '../types.js';
import { ChrisSprite } from './ChrisSprite.js';
import { Button } from './Button.js';

export const GameEngine = ({ onGameOver, onRestart }) => {
  // Game State Refs
  ...
  // Power-up states
  const powerUpActive = useRef(null);
  ...
  const playMusicNote = (time, noteIndex) => {
  ...
  const activatePowerUp = (type) => {
    ...
    } else if (type === 'CHRISTMAS_TRIGGER') {
  ...
  const ElfSprite = ({x, y, reverse = false}) => (
