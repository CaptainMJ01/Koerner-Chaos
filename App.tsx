...
import { GameState, LeadInfo, HighScoreEntry } from './types';
import { UI_COPY, GAME_WIDTH, GAME_HEIGHT, HIGH_SCORE_KEY } from './constants';
...
const App: React.FC = () => {
  ...
  const [userData, setUserData] = useState<LeadInfo | null>(null);
  const [highScores, setHighScores] = useState<HighScoreEntry[]>([]);
  // Game ID used as key to force full remount of GameEngine on restart
  ...
  const handleLeadSubmit = (data: LeadInfo | null) => {
    setUserData(data);
    if (data) {
  ...
  const handleGameOver = (score: number) => {
    ...
    if (userData) {
      const newEntry: HighScoreEntry = { name: userData.name, score };
